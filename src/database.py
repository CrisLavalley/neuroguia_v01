...
LOCAL_DB = BASE / "data" / "runtime" / "neuroguia_v08.db"
...
def get_engine() -> Engine:
    ...
    return create_engine(url, future=True, pool_pre_ping=True, poolclass=NullPool, connect_args={"prepare_threshold": None})

def init_db(schema_path: Optional[Path] = None) -> None:
    engine = get_engine()
    schema_path = schema_path or (BASE / "sql" / "schema.sql")
